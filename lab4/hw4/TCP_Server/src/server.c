...
int main(int argc, char *argv[]) {
    ...
    // Create folder
    sprintf(full_directory, "%s/%s", BASE_DIRECTORY, directory_name);
    if (mkdir(full_directory, 0777) == -1) {
        printf("Directory %s already exists, no need to create a new directory\n", directory_name);
    }
